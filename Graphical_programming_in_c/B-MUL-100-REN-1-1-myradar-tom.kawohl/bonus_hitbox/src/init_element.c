...
void update_scale(object_t **planes, int i, int width, int height)
{
    sfVector2f scale = {1, 1};
    scale.x = (float)width / 20;
    scale.y = (float)height / 20;
    sfSprite_scale(planes[i]->sprite, scale);
}
...
void init_plane(object_t **planes, int i, char **map_info, int *j)
{
    ...
    planes[i]->hitbox = (sfIntRect){0, 0, plane_info[6], plane_info[7]};
    update_scale(planes, i, plane_info[6], plane_info[7]);
